...
#Step-3 ___Image read as __image
im = Image.open(__image)					# Crop from image for openCV


#Step-4 ___Load plugin and cpu extension libraly for 2019R3 only
...
#Step-5 ___Set IENetwork with xml , bin
net = IENetwork(model=__xml,weights=__bin)

#Step-6 ___Set exec_net and load to network   
exec_net = plugin.load(network=net)



#Step-7 ___Read image and format for openvino format
...
#Step-8 ___ execute inference engine with image and get result as "res" start sync mode
...
#Step-9 ___'detection_out' is result from inference engine from res
res = res['detection_out']	
res = np.squeeze(res) 		

#Step-10 ___ Judge if res[2] is over 0.5 , append into values
values=[]
